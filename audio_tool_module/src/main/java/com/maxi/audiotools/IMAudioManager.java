...
public class IMAudioManager {
    ...
    //是否自动缓存文件
    private boolean isAutoCache = true;
     ...
     * 播放音频
     *
     * @param voicePath            网络或者本地音频地址
     * @param isAutoCache          网络音频是否自动缓存
     * @param onCompletionListener 播放监听
     */
    public void playSound(String voicePath, boolean isAutoCache, MediaPlayer.OnCompletionListener onCompletionListener) {
        this.isAutoCache = isAutoCache;
        playSound(voicePath, onCompletionListener);
    ...
    public void playSound(String voicePath,
        ...
        try {
            ...
            if (path != null) { // 存在缓存文件
                ...
                // 异步下载音频文件
                if (isAutoCache) new AudioAsyncTask(fileUtils).execute(voicePath);
